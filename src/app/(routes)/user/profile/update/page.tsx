...
const Page = () => {
  ...
  const { customer, loading, user: supabaseUser, error, refetch } = useUser();
  ...
  useEffect(() => {
    if (customer && !loading) {
      ...
      setFirstName(supabaseUser?.user_metadata?.first_name || "");
      setLastName(supabaseUser?.user_metadata?.last_name || "");
      // Clear DOB fields for new users or if customer data is absent
      setDobDay("");
      setDobMonth("");
      setDobYear("");
    }
  }, [customer, loading, supabaseUser]);
  ...
  // --- Loading State and Conditional Render ---
  if (loading) {
    return (
  ...
  if (!supabaseUser) {
    // This return is effectively handled by useEffect redirect, but good for safety
    return null;
  }
        ...
        <div className="lg:col-span-4 space-y-6 pr-20">
          <div className="grid w-full items-center gap-2">
            ...
            <Input
              ...
              className="w-full"
              disabled={!!customer?.username || isSavingProfile} // Disable if username set or saving
            />
            {customer?.username && (
          ...
          <div className="grid w-full items-center gap-2">
            ...
            <div className="flex items-center">
              {maskEmail(supabaseUser?.email || customer?.email)}
            </div>
          </div>
